  ...
  fields: [
    ...
    },
    {
      ...
      fields: [
        ...
        { 
          ...
          hidden: ({ parent }: { parent: unknown }) => (parent as Record<string, unknown>)?.titleDisplayMode === 'none'
        },
        { 
          name: 'altDescription', 
    ...
    },
    {
      ...
      fields: [
        ...
        { name: 'overlayOpacity', title: 'Overlay Opacity', type: 'number', description: 'Opacity for the overlay color (0 to 1).', hidden: ({ parent }: { parent: unknown }) => !(parent as Record<string, unknown>)?.overlayColor },
    ...
    },
    {
      ...
      fields: [
        ...
        { name: 'size', title: 'Size', type: 'string', options: { list: ['auto', 'small', 'medium', 'large', 'full', 'custom'] }, description: 'Set the max width of the image. Example: "small" (max-w-xs), "medium" (max-w-md), "large" (max-w-lg), "full" (100% width). Use "custom" with Width/Max Width for advanced sizing.', hidden: ({ parent }: { parent: unknown }) => (parent as Record<string, unknown>)?.fullBleed },
        { name: 'width', title: 'Width', type: 'string', description: 'Custom width (overrides size). Example: "300px", "50vw", "20rem".', hidden: ({ parent }: { parent: unknown }) => (parent as Record<string, unknown>)?.fullBleed || (parent as Record<string, unknown>)?.size !== 'custom' },
        { name: 'height', title: 'Height', type: 'string', description: 'Custom height. Example: "200px", "30vh", "15rem". Leave blank for auto height.', hidden: ({ parent }: { parent: unknown }) => (parent as Record<string, unknown>)?.fullBleed || (parent as Record<string, unknown>)?.size !== 'custom' },
