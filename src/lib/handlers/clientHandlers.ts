...
export async function getSvgAssets(query?: Partial<SvgAssetQuery>): Promise<SvgAsset[]> {
  try {
    ...
    if (query?.order !== undefined) params.append('order', query.order.toString());
...
export async function getVideoAsset(query: VideoAssetQuery): Promise<VideoAsset | null> {
  const { id } = query;
...
export function getVideoSources(asset: VideoAsset): { src: string; type: string; quality: string }[] {
  const sources = [];
...
export function getColoredSvg(svgData: string, color: string): string {
  ...
  cleanedSvg = cleanedSvg
    .replace(/<(path|rect|circle|ellipse|polygon|polyline|line)/g, `<$1 fill="${color}" stroke="${color}"`)
    .replace(/<(path|rect|circle|ellipse|polygon|polyline|line)([^>]*?)(\/>|>)/g, (match, tag, attrs, end) => {
