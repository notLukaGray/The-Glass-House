...
export async function getImageAssetServer({ id }: { id: string }): Promise<ImageAsset | null> {
  try {
...
export async function getSvgAssetServer({ id }: { id: string }): Promise<SvgAsset | null> {
  try {
    const client = createSanityClient();
...
export async function getVideoAssetServer({ id }: { id: string }): Promise<VideoAsset | null> {
  try {
    const client = createSanityClient();
    ...
    const url = videoAsset.cdn4kUrl || videoAsset.cdn2kUrl || videoAsset.cdn1kUrl || videoAsset.cdnSdUrl;
